...
const projectPlanStatusMeta={
  ...
  '已完成':{className:'delivered',description:'生产计划已完成全部生产'}
};
function syncProjectPlanProductionProgress(project){for(const batch of project.batches||[]){const works=(project.works||[]).filter(work=>String(work.planId)===String(batch.id));batch.completed=works.filter(work=>projectWorkStatusLabel(work.status)==='已完成').length;batch.productionStatus=works.length>0&&batch.completed===works.length?'已完成':'生产中';batch.status=batch.productionStatus}}
...
function projectWorkStatusLabel(status){return ['已完成','已交付'].includes(status)?'已完成':'制作中'}
function projectWorkStatusClass(status){return {已完成:'completed',制作中:'making'}[projectWorkStatusLabel(status)]}
...
function taskAssigneeForNode(context,node){
  ...
  const assignment=context.work.nodeAssignments?.[key]||context.work.nodeAssignments?.[`${context.plan.workflowId}:${context.plan.workflowVersion}:${node.sourceNodeId}`];
  return {id:assignment?.memberId||'',name:assignment?.memberName||context.work.owner||'待分配'};
}
...
function taskNodeActionLabel(task){
  if(['lyricsReview','songReview'].includes(task.nodeType))return '确认审核并进入下一节点';
  if(task.nodeType==='refLyrics')return '确认歌词并进入歌词审核';
  if(task.nodeType==='refSong')return '确认歌曲并进入歌曲审核';
  if(['aiLyrics','refLyrics','aiSong','refSong','sampleSong','playlistSong'].includes(task.nodeType))return '运行并进入下一节点';
  return '提交并进入下一节点';
}
...
function renderTaskCenter(){
  ...
  document.querySelector('#taskListTitle').textContent=currentTaskStatus==='active'?'进行中任务':currentTaskStatus==='finished'?'已完成任务':'全部';
...
const businessCustomers=[
  {name:'星河品牌',company:'星河品牌管理有限公司',type:'平台方',businessLine:'品牌年度音乐与短视频传播',demands:['全案','人声','混音'],contact:'张三',status:'长期合作中',priority:'P1',notes:'年度合作方案已确认，进入持续制作阶段；企微群：星河年度音乐项目。',code:'OP001',enabled:true,orders:4,projects:6,last:'2026-09-03'},
  {name:'城市文化中心',company:'城市文化传播中心',type:'版权方',businessLine:'城市活动配乐与公共文化内容',demands:['全案','伴奏'],contact:'Lin Chen',status:'制作进行',priority:'P1',notes:'音乐节片头已进入第一轮制作；飞书项目群：城市音乐节。',code:'MP001',enabled:true,orders:3,projects:4,last:'2026-09-01'},
  {name:'北岛工作室',company:'北岛音乐工作室',type:'制作方',businessLine:'独立音乐人专辑制作',demands:['人声','混音','伴奏'],contact:'Ming Xu',status:'持续合作',priority:'P2',notes:'本季度专辑合作按计划推进；每周三同步制作进展。',code:'MP002',enabled:true,orders:6,projects:9,last:'2026-08-28'},
  {name:'极光传媒',company:'极光新媒体有限公司',type:'渠道方',businessLine:'短视频音乐授权与分发',demands:['抢热度','分发端口','销售渠道'],contact:'张三',status:'合同签署',priority:'P2',notes:'合同已完成法务审核，等待盖章；合同链接：内部协作空间。',code:'MP003',enabled:true,orders:8,projects:11,last:'2026-08-20'},
  {name:'镜像互动',company:'镜像互动科技有限公司',type:'制作方',businessLine:'互动内容与生成式音乐实验',demands:['DJ','其他'],contact:'Lin Chen',status:'初步接触',priority:'P3',notes:'已完成首次需求沟通，等待技术方案；会议纪要：互动音乐方案沟通。',code:'MP004',enabled:false,orders:2,projects:2,last:'2026-08-16'}
];
...
const customerDisableDialog=document.querySelector('#customerDisableDialog');
let pendingDisableCustomer=null;
function syncCustomerAvailability(){[projectCustomer,newOrderCustomer].forEach(select=>Array.from(select.options).forEach(option=>{const customer=businessCustomers.find(item=>item.name===option.textContent);option.disabled=customer?.enabled===false;if(option.disabled&&option.selected)select.value=''}))}
customerDisableDialog.addEventListener('close',()=>{const customer=pendingDisableCustomer;pendingDisableCustomer=null;if(customerDisableDialog.returnValue!=='confirm'||!customer)return;customer.enabled=false;syncCustomerAvailability();renderBusiness();showToast(`客户「${customer.name}」已禁用`)});
...
const businessPageSize=document.querySelector('#businessPageSize');
function renderBusinessPagination(total){
  const size=Number(businessPageSize.value),pages=Math.max(1,Math.ceil(total/size));
  businessCurrentPage=Math.min(Math.max(1,businessCurrentPage),pages);
  const start=(businessCurrentPage-1)*size;
  document.querySelector('#businessPageSummary').textContent=total?`共 ${total} 条，显示 ${start+1}–${Math.min(start+size,total)} 条`:'共 0 条';
  const button=(page,label,disabled=false)=>`<button type="button" data-business-page="${page}" ${disabled?'disabled':''} ${label===String(businessCurrentPage)?'aria-current="page"':''}>${label}</button>`;
  const numbers=[...new Set([1,pages,businessCurrentPage-1,businessCurrentPage,businessCurrentPage+1])].filter(n=>n>=1&&n<=pages).sort((a,b)=>a-b);
  document.querySelector('#businessPageButtons').innerHTML=button(businessCurrentPage-1,'上一页',businessCurrentPage===1)+numbers.map((n,i)=>(i&&n-numbers[i-1]>1?'<span aria-hidden="true">…</span>':'')+button(n,String(n))).join('')+button(businessCurrentPage+1,'下一页',businessCurrentPage===pages);
  return start;
}
function resetBusinessPagination(){businessCurrentPage=1;renderBusiness()}
businessPageSize.addEventListener('change',resetBusinessPagination);
document.querySelector('#businessPageButtons').addEventListener('click',event=>{const button=event.target.closest('[data-business-page]');if(!button||button.disabled)return;businessCurrentPage=Number(button.dataset.businessPage);renderBusiness();document.querySelector('#businessPageButtons [aria-current="page"]')?.focus({preventScroll:true})});
...
businessTable.addEventListener('click',event=>{
  ...
  if(customer.enabled!==false){pendingDisableCustomer=customer;customerDisableDialog.returnValue='cancel';customerDisableDialog.showModal();return}
  customer.enabled=true;
  syncCustomerAvailability();
  renderBusiness();
...
function reserveDailyProductionNumbers(counts,now=new Date()){
  const day=localDateValue(now).replaceAll('-',''),storageKey='orchestra-daily-production-numbers-v1';
  const minute=day+String(now.getHours()).padStart(2,'0')+String(now.getMinutes()).padStart(2,'0');
  const prefixes={project:`PROJECT-${day}-`,plan:`BATCH-${day}-`,flow:`FLOW${day}`,work:minute};
  ...
  try{
    ...
    Object.entries(counts).forEach(([kind,count])=>{
      ...
      if(!Number.isSafeInteger(start+count)||(kind==='work'&&start+count>9999999))throw Error('编号已超出范围');
      result[kind]=Array.from({length:count},(_,index)=>prefix+String(start+index+1).padStart(kind==='work'?7:5,'0'));
...
function ensureProjectWorkNumbers(){
  ...
  try{
    const saved=JSON.parse(localStorage.getItem(storageKey)||'{}'),entries=projectData.flatMap(project=>(project.works||[]).map(work=>({project,work,key:JSON.stringify([project.id,String(work.id)])}))),used=new Set(),missing=[];
    entries.forEach(entry=>{const {work,key}=entry,record=saved[key],candidate=record?.workNumber||work.workNumber;if(typeof candidate==='string'&&/^\d{19}$/.test(candidate)&&!used.has(candidate)){work.workNumber=candidate;used.add(candidate)}else missing.push(entry);if(record?.updatedAt&&!/^\d{4}[-/]/.test(work.updated||''))work.updated=record.updatedAt});
    if(missing.length){const numbers=reserveDailyProductionNumbers({work:missing.length});if(!numbers)return false;missing.forEach(({work},index)=>work.workNumber=numbers.work[index])}
...
function createPlanWorks(plan,reservedSerials,reservedWorkNumbers){
  ...
  return serials.map((serial,index)=>({id:serial,serial,workNumber:workNumbers[index],name:`未命名-${serial.slice(-4)}`,planId:plan.id,planName:plan.name,status:'制作中',stage:'基于参考生成歌词',lyrics:'暂无',audio:'暂无',cover:'暂无',owner:'未分配',createdAt:now.toISOString(),updated:now.toISOString()}));
}
...
const projectPlanTemplate=projectPlanGroups.querySelector('[data-project-plan-group]').cloneNode(true);
let projectPlanGroupSequence=0;
const MAX_PROJECT_PLAN_GROUPS=5;
function populatePublishedWorkflows(){projectPlanGroups.querySelectorAll('[name="workflow"]').forEach(select=>{const previous=select.value,published=workflowData.filter(item=>item.publishClass==='published');select.innerHTML='<option value="">请选择已发布工作流</option>';published.forEach(item=>{const option=document.createElement('option');option.value=item.id;option.textContent=`${item.name} · ${item.version}`;select.append(option)});if(published.some(item=>item.id===previous))select.value=previous})}
function updateProjectPlanGroupTitles(){const groups=Array.from(projectPlanGroups.children);groups.forEach((group,index)=>{group.querySelector('[data-plan-group-title]').textContent=`生产计划 ${index+1}`;group.querySelector('[data-remove-plan-group]').disabled=index===0;group.querySelector('[data-remove-plan-group]').setAttribute('aria-label',`删除生产计划 ${index+1}`)});const addButton=document.querySelector('#addProjectPlanGroup');addButton.disabled=groups.length>=MAX_PROJECT_PLAN_GROUPS;addButton.textContent=addButton.disabled?'已达上限（最多 5 组生产计划）':'＋ 添加一组生产计划（最多 5 组）'}
document.querySelector('#addProjectPlanGroup').addEventListener('click',()=>{
  if(projectPlanGroups.children.length>=MAX_PROJECT_PLAN_GROUPS){showToast('最多创建 5 组生产计划');return}
  const group=projectPlanTemplate.cloneNode(true),suffix=`-group-${++projectPlanGroupSequence}`,ids=new Map();
  group.querySelectorAll('[id]').forEach(element=>{ids.set(element.id,element.id+suffix);element.id+=suffix});
  group.querySelectorAll('[for]').forEach(label=>{if(ids.has(label.htmlFor))label.htmlFor=ids.get(label.htmlFor)});
  projectPlanGroups.append(group);populatePublishedWorkflows();updateProjectPlanGroupTitles();group.querySelector('[name="productionName"]').focus();
});
projectPlanGroups.addEventListener('click',event=>{const button=event.target.closest('[data-remove-plan-group]');if(!button||button.disabled)return;const group=button.closest('[data-project-plan-group]'),previous=group.previousElementSibling;group.remove();updateProjectPlanGroupTitles();previous?.querySelector('[name="productionName"]').focus()});
projectPlanGroups.addEventListener('input',event=>{if(event.target.name!=='productionName')return;const input=event.target;input.value=Array.from(input.value).slice(0,50).join('');input.closest('[data-project-plan-group]').querySelector('[data-plan-name-count]').textContent=`${Array.from(input.value).length} / 50`;input.closest('.project-field').classList.remove('is-invalid')});
...
function collectProjectPlanDrafts(){
  if(projectPlanGroups.children.length<1||projectPlanGroups.children.length>MAX_PROJECT_PLAN_GROUPS){showToast('请添加 1～5 组生产计划');return null}
  const drafts=[],names=new Set();
  for(const [index,group] of Array.from(projectPlanGroups.children).entries()){
    const nameInput=group.querySelector('[name="productionName"]'),quantityInput=group.querySelector('[name="quantity"]'),workflowInput=group.querySelector('[name="workflow"]'),name=nameInput.value.trim(),quantity=Number(quantityInput.value),workflow=workflowData.find(item=>item.id===workflowInput.value&&item.publishClass==='published'),prefix=`生产计划 ${index+1}：`;
    const invalid=(control,message)=>{invalidateProjectField(control,prefix+message);return null};
    if(!name||Array.from(name).length>50)return invalid(nameInput,'请输入 1～50 字的生产计划名称');
    if(names.has(name))return invalid(nameInput,'生产计划名称不能重复');
    if(!Number.isSafeInteger(quantity)||quantity<1)return invalid(quantityInput,'生产数量必须为正整数');
    if(!workflow)return invalid(workflowInput,'请选择已发布工作流');
    names.add(name);drafts.push({name,quantity,workflow,delivery:group.querySelector('[name="deliveryDate"]').value,notes:sanitizeRichText(group.querySelector('.rich-input').innerHTML)});
  }
  return drafts;
}
function buildInitialProjectPlans(drafts,owner,numbers){
  let offset=0;const works=[];
  const batches=drafts.map((draft,index)=>{const batch={id:numbers.plan[index],name:draft.name,quantity:draft.quantity,completed:0,delivered:0,terminated:0,inProgress:0,owner,workflowId:draft.workflow.id,workflowName:draft.workflow.name,workflowVersion:draft.workflow.version,start:localDateValue(new Date()),delivery:draft.delivery||'待排期',priority:'普通',status:'待生产',progress:0,notes:draft.notes};works.push(...createPlanWorks(batch,numbers.flow.slice(offset,offset+draft.quantity),numbers.work.slice(offset,offset+draft.quantity)));offset+=draft.quantity;return batch});
...
function updateCustomerNotesCount(){let characters=Array.from(richTextPlainText(newCustomerNotes.innerHTML));if(characters.length>1000){newCustomerNotes.textContent=characters.slice(0,1000).join('');characters=Array.from(richTextPlainText(newCustomerNotes.innerHTML));const range=document.createRange(),selection=window.getSelection();range.selectNodeContents(newCustomerNotes);range.collapse(false);selection.removeAllRanges();selection.addRange(range);showToast('备注最多输入 1000 个字符','error')}newCustomerNotesCount.textContent=`${characters.length} / 1000`}
...
newCustomerForm.onsubmit=event=>{
  ...
  if(!demands.length){customerDemandField.classList.add('is-invalid');setCustomerDemandOpen(true);customerDemandTrigger.focus({preventScroll:true});customerDemandTrigger.scrollIntoView({behavior:'smooth',block:'center'});showToast('请至少选择一项制作需求','error');return}
  ...
  if(Array.from(notesText).length>1000){newCustomerNotes.focus({preventScroll:true});showToast('备注最多输入 1000 个字符','error');return}
  const payload={name,company:newCustomerCompany.value.trim(),type:newCustomerType.value,businessLine:newCustomerBusinessLine.value.trim(),demands,contact:newCustomerContact.value,priority:newCustomerPriority.value,notes:notesText?notesHtml:'',status:newCustomerProgressStatus.value};
  if(editingCustomer){
...
projectCreateForm.onsubmit=event=>{
  ...
  if(businessCustomers.some(item=>item.name===projectCustomer.selectedOptions[0]?.textContent&&item.enabled===false))return invalidateProjectField(projectCustomer,'该客户已禁用，请选择其他客户');
  ...
  const totalQuantity=drafts.reduce((total,draft)=>total+draft.quantity,0);
  if(!Number.isSafeInteger(totalQuantity)||totalQuantity<1){showToast('生产总数量无效，请检查各组生产数量','error');return}
  ...
  const deliveryDate=drafts.map(draft=>draft.delivery).filter(Boolean).sort().at(-1)||'';
  const orderNotes=document.querySelector('#projectOrderNotes').innerHTML.trim();
  const productionPlanNotes=drafts.map(draft=>draft.notes).filter(Boolean).join('<br>');
  ...
  const description=`${drafts.length} 组生产计划 · ${totalQuantity} 份`;
  ...
  startProductionPlanTasks(projectData[0],initialPlans.works);
  renderBusiness();
  setProjectFilter('all');
  switchPage('projects',document.querySelector('.nav-item[data-page="projects"]'));
  showToast(`项目「${projectName}」创建成功，已创建 ${drafts.length} 组生产计划、${totalQuantity} 个作品`);
};
