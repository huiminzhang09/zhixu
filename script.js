...
projectData.forEach((project,index)=>{const batch=project.batches[0];project.runs=[{name:index%2===0?`${project.type}样片`:'',planId:batch.id,planName:batch.name,serial:`FLOW20260907${String(index+4).padStart(5,'0')}`,status:batch.status==='已完成'?'已完成':'制作中',stage:batch.status==='已完成'?'已完成':'作曲',lyrics:'查看',audio:batch.completed>0?'播放':'暂无',cover:batch.status==='已完成'?'查看':'暂无',owner:batch.owner,updated:`09-0${Math.min(7,index+2)} ${15-index}:30`}]});
projectData[0].runs=[
...
projectData[0].runs.unshift({id:'complete-reference-work',name:'参考词曲创作任务',planId:'BATCH-202609-REF',planName:'参考词曲完整任务',serial:'FLOW2026090800001',status:'制作中',stage:'基于参考生成歌词',lyrics:'暂无',audio:'暂无',cover:'暂无',owner:'李四',updated:'今天 10:30',taskReached:0,taskFeatured:true});
...
projectData.forEach(project=>(project.runs||[]).forEach((work,index)=>{work.id=work.id||work.serial||`${project.id}-work-${index+1}`;const saved=savedProjectWorkAssignments[`${project.id}:${work.id}`];if(saved){work.nodeAssignments=saved.nodeAssignments||{};work.lastAssigneeId=saved.lastAssigneeId||'';work.lastAssignmentNode=saved.lastAssignmentNode||'';work.owner=saved.owner||work.owner;work.updated=saved.updated||work.updated}}));
function persistProjectWorkAssignments(){const payload={};projectData.forEach(project=>(project.runs||[]).forEach(work=>{if(!work.nodeAssignments||!Object.keys(work.nodeAssignments).length)return;payload[`${project.id}:${work.id}`]={nodeAssignments:work.nodeAssignments,lastAssigneeId:work.lastAssigneeId,lastAssignmentNode:work.lastAssignmentNode,owner:work.owner,updated:work.updated}}));try{localStorage.setItem(projectWorkAssignmentStorageKey,JSON.stringify(payload));return true}catch{return false}}
const projectWorkNamesStorageKey='orchestra-project-work-names-v1';
try{const names=JSON.parse(localStorage.getItem(projectWorkNamesStorageKey)||'{}');projectData.forEach(project=>(project.runs||[]).forEach(work=>{const record=names[JSON.stringify([project.id,String(work.id)])],name=typeof record==='string'?record:record?.name;if(typeof name==='string'&&name.trim())work.name=name;if(record?.updatedAt&&(!/^\d{4}[-/]/.test(work.updated||'')||new Date(record.updatedAt)>new Date(work.updated)))work.updated=record.updatedAt}))}catch{}
...
function projectAssignableMembers(project){const target=Math.max(1,projectMemberCount(project)),members=[];const add=member=>{if(member&&!members.some(item=>item.id===member.id)&&members.length<target)members.push(member)};(project.members||[]).filter(mark=>!mark.startsWith('+')).forEach(mark=>add(teamMemberRoster.find(member=>member.mark===mark)));(project.runs||[]).forEach(work=>add(teamMemberRoster.find(member=>member.name===work.owner)));teamMemberRoster.forEach(add);return members}
...
function syncProjectPlanProductionProgress(project){
  for(const batch of project.batches||[]){
    const works=(project.runs||[]).filter(work=>String(work.planId)===String(batch.id));
...
function projectWorkName(project,work){const unnamed=(project.runs||[]).filter(item=>!String(item.name||'').trim()),index=Math.max(0,unnamed.indexOf(work));return projectWorkDisplayName(work,index)}
function visibleProjectWorks(project){const works=project.runs||[];return currentProjectWorksPlan==='all'?works:works.filter(work=>String(work.planId)===currentProjectWorksPlan)}
...
function renderProjectWorks(project){const plans=project.batches||[],planNameMap=new Map(plans.map(plan=>[String(plan.id),plan.name||'未命名生产计划'])),validPlanIds=new Set(planNameMap.keys());if(currentProjectWorksPlan!=='all'&&!validPlanIds.has(currentProjectWorksPlan))currentProjectWorksPlan='all';projectWorksPlanFilter.innerHTML=`<option value="all">全部生产计划</option>${plans.map(plan=>`<option value="${escapeHtml(plan.id)}">${escapeHtml(plan.name||'未命名生产计划')}</option>`).join('')}`;projectWorksPlanFilter.value=currentProjectWorksPlan;const allWorks=project.runs||[],unnamedOrder=new Map();let unnamedIndex=0;allWorks.forEach((work,index)=>{work.id=work.id||work.serial||`${project.id}-work-${index+1}`;if(!String(work.name||'').trim())unnamedOrder.set(work,unnamedIndex++)});const validWorkIds=new Set(allWorks.map(work=>String(work.id)));selectedProjectWorkIds=new Set([...selectedProjectWorkIds].filter(id=>validWorkIds.has(id)));const works=visibleProjectWorks(project),rows=works.map(work=>projectWorkRow(work,unnamedOrder.get(work)||0,planNameMap,project));projectWorksGrid.innerHTML=`<div class="project-work-row head" role="row"><div class="project-work-select-cell" role="columnheader"><label class="project-work-select"><input type="checkbox" data-work-select-all aria-label="全选当前筛选结果" /></label></div><span role="columnheader">运行名称</span><span role="columnheader">所属生产计划</span><span role="columnheader">运行编号</span><span role="columnheader">状态</span><span role="columnheader">当前执行节点</span><span role="columnheader">负责人</span><span role="columnheader">歌词</span><span role="columnheader">音频</span></div>${rows.join('')}`;projectWorksResultCount.textContent=`${works.length} 个运行`;projectWorksTableWrap.hidden=!works.length;projectWorksEmpty.hidden=Boolean(works.length);projectWorksEmptyTitle.textContent=currentProjectWorksPlan==='all'?'暂无运行':'该生产计划暂无运行';projectWorksEmptyDescription.textContent=currentProjectWorksPlan==='all'?'生产开始后，运行会在这里持续沉淀。':'可切换其他生产计划查看已生成的运行。';syncProjectWorkSelectionUI(works)}
function selectedProjectWorks(project){return (project.runs||[]).filter(work=>selectedProjectWorkIds.has(String(work.id)))}
...
function openProjectWorkAssignmentDialog(){const project=projectData.find(item=>item.id===selectedProjectId),works=project?selectedProjectWorks(project):[];if(!project||!works.length){showToast('请先勾选需要分配的运行','error');return}const contexts=works.map(work=>projectWorkAssignmentContext(project,work));if(contexts.some(context=>!context)){showToast('部分运行未关联生产计划，暂无法分配','error');return}const signatures=new Set(contexts.map(context=>`${context.plan.workflowId}:${context.plan.workflowVersion}`));if(signatures.size!==1){showToast('批量分配需选择同一工作流版本下的运行','error');return}const snapshot=contexts[0].snapshot;if(!snapshot){showToast('未找到该生产计划绑定的已发布工作流版本','error');return}if(!snapshot.nodes.length){showToast('该工作流版本暂无可分配节点','error');return}const members=projectAssignableMembers(project);if(!members.length){showToast('当前项目暂无可分配成员','error');return}activeProjectWorkAssignment={project,works,contexts,snapshot,members};projectWorkAssignmentReturnFocus=document.activeElement;projectWorkAssignmentForm.reset();projectWorkAssignmentAllMember.innerHTML='<option value="">选择后应用到全部节点</option>'+members.map(member=>`<option value="${escapeHtml(member.id)}">${escapeHtml(member.name)} · ${escapeHtml(member.role)}</option>`).join('');populateProjectWorkAssignmentNodes(snapshot.nodes,members,contexts);document.querySelector('#projectWorkAssignmentCount').textContent=`已选择 ${works.length} 个运行`;const planNames=[...new Set(contexts.map(context=>context.plan.name))];document.querySelector('#projectWorkAssignmentWorkflow').textContent=`${planNames.join('、')} · ${contexts[0].plan.workflowName} ${contexts[0].plan.workflowVersion}`;document.querySelector('#projectWorkAssignmentNames').innerHTML=works.map(work=>`<span title="${escapeHtml(projectWorkName(project,work))}">${escapeHtml(projectWorkName(project,work))}</span>`).join('');document.querySelector('#projectWorkAssignmentDescription').textContent=`为已选择的 ${works.length} 个运行配置节点分工，每个环节只能选择一位负责人。`;projectWorkAssignmentNodeHint.textContent=`当前工作流：${contexts[0].plan.workflowName} ${contexts[0].plan.workflowVersion}（已发布，共 ${snapshot.nodes.length} 个节点）`;projectWorkAssignmentDialog.hidden=false;document.body.classList.add('order-dialog-open');requestAnimationFrame(()=>projectWorkAssignmentNodeList.querySelector('[data-node-assignee]')?.focus({preventScroll:true}))}
...
function buildDefaultProductionTasks(){
  ...
  let sequence=0;
  projectData.forEach((project,projectIndex)=>(project.runs||[]).forEach((work,workIndex)=>{
...
function removedProductionTaskWorks(saved){
  ...
  const entries=projectData.flatMap(project=>(project.runs||[]).map(work=>({project,work,key:taskWorkKey(project.id,work.id)})));
...
function loadProductionTasks(){
  ...
  saved.filter(task=>!defaultIds.has(task.id)&&(task.featured||task.assignmentCreated)&&projectData.some(project=>project.id===task.projectId&&(project.runs||[]).some(work=>String(work.id)===String(task.workId)))).forEach(task=>productionTasks.push(task));
  const featuredWork=projectData.flatMap(project=>(project.runs||[]).map(work=>({project,work}))).find(item=>item.work.taskFeatured);
  if(featuredWork){
  ...
  projectData.forEach(project=>ensureAssignedProductionTasks(project,(project.runs||[]).filter(work=>Object.keys(work.nodeAssignments||{}).length&&!removedWorks.has(taskWorkKey(project.id,work.id)))));
  persistProductionTasks();
}
...
function advanceProductionTask(task,outcome){
  ...
  task.status=outcome==='complete'?'completed':'terminated';if(outcome==='complete'){task.actualSubmitter=task.assigneeName;task.completedAt=new Date().toISOString();}task.updatedAt=Date.now();task.updatedLabel='刚刚';
  let nextTask=null;
  if(outcome==='complete'){
    if(['lyricsReview','songReview'].includes(task.nodeType)){
...
function initializeTaskCenter(){loadProductionTasks();taskCenterReady=true;renderTaskCenter()}
...
// Resolve legacy display names once; subsequent customer relationships use stable codes.
businessOrders.forEach(order=>{if(!order.customerCode)order.customerCode=businessCustomers.find(customer=>customer.name===order.customer)?.code||''});
projectData.forEach(project=>{if(!project.customerCode){const order=businessOrders.find(item=>item.id===project.orderId);project.customerCode=businessCustomers.find(customer=>customer.name===project.customer)?.code||order?.customerCode||''}});
...
// The numbering day begins at 00:00 Asia/Shanghai.
function flowNumberingDate(value=new Date()){return beijingDateKey(value)}
function reserveDailyProductionNumbers(counts,now=new Date()){
 const day=beijingDateKey(now),flowDay=flowNumberingDate(now),storageKey='orchestra-daily-production-numbers-v1';
 const prefixes={project:`PROJECT-${day}-`,plan:`BATCH-${day}-`,flow:`FLOW${flowDay}`,work:day};
 const existing={project:projectData.map(p=>p.id),plan:projectData.flatMap(p=>(p.batches||[]).map(p=>p.id)),flow:projectData.flatMap(p=>(p.runs||[]).map(r=>r.serial)),work:projectData.flatMap(p=>(p.works||[]).map(w=>w.id))};
 try{
  const saved=JSON.parse(localStorage.getItem(storageKey)||'{}'),result={};
  for(const [kind,count] of Object.entries(counts)){
   if(!prefixes[kind]||!Number.isSafeInteger(count)||count<1)throw Error('编号数量无效');
   const key=kind==='flow'?flowDay:day,prefix=prefixes[kind],counters=saved[key]||={};
   ...
   const start=Math.max(latest,Number(counters[kind])||0),limit=kind==='work'?999999:kind==='flow'?99999:Number.MAX_SAFE_INTEGER;
   if(!Number.isSafeInteger(start+count)||start+count>limit)throw Error(kind==='flow'?'当日运行编号已达上限（99999），请联系管理员。':'编号已超出范围');
   result[kind]=Array.from({length:count},(_,i)=>prefix+String(start+i+1).padStart(kind==='work'?6:5,'0'));counters[kind]=start+count;
  }
  localStorage.setItem(storageKey,JSON.stringify(saved));return result;
 }catch(error){showToast(error.message.includes('99999')?error.message:'编号保存失败，请检查浏览器存储后重试','error');return null}
}
function ensureProjectWorkNumbers(){return true}
function createPlanWorks(plan,reservedSerials){
 const quantity=Number(plan.quantity);if(!Number.isSafeInteger(quantity)||quantity<1)throw Error('计划运行次数须为大于0的整数。');
 const serials=reservedSerials||reserveDailyProductionNumbers({flow:quantity})?.flow;
 if(!serials||serials.length!==quantity)throw Error('运行编号生成失败');
 return serials.map((serial,index)=>({id:serial,serial,name:`${plan.name} · 运行 ${index+1}`,planId:plan.id,planName:plan.name,status:'制作中',stage:'等待人员分配',lyrics:'暂无',audio:'暂无',owner:'未分配',createdAt:new Date().toISOString(),updated:new Date().toISOString()}));
}
...
projectBatchForm.onsubmit=event=>{event.preventDefault();const project=projectData.find(item=>item.id===selectedProjectId);if(!project){showToast('未找到当前项目','error');return}const name=newBatchName.value.trim();if(!name)return requireOrderControl(newBatchName,'请输入计划名称');if(project.batches.some(batch=>batch.name===name)){showToast('当前项目中已存在同名计划','error');newBatchName.focus({preventScroll:true});return}if(!projectBatchQuantity.value.trim())return requireOrderControl(projectBatchQuantity,'请输入计划运行次数。');if(!Number.isSafeInteger(Number(projectBatchQuantity.value))||Number(projectBatchQuantity.value)<1)return requireOrderControl(projectBatchQuantity,'计划运行次数须为大于0的整数。');if(!projectBatchWorkflow.value)return requireOrderControl(projectBatchWorkflow,'请选择已发布工作流');const workflow=workflowData.find(item=>item.id===projectBatchWorkflow.value);if(!workflow)return requireOrderControl(projectBatchWorkflow,'请选择有效的已发布工作流');const numbers=reserveDailyProductionNumbers({plan:1,flow:Number(projectBatchQuantity.value)});if(!numbers)return;project.batches.unshift({id:numbers.plan[0],name,quantity:Number(projectBatchQuantity.value),completed:0,delivered:0,terminated:0,inProgress:0,owner:'未分配',workflowId:workflow.id,workflowName:workflow.name,workflowVersion:workflow.version,start:localDateValue(new Date()),delivery:projectBatchDelivery.value||'待排期',priority:'普通',status:'待生产',progress:0,notes:projectBatchNotes.innerHTML.trim()});const newWorks=createPlanWorks(project.batches[0],numbers.flow,numbers.work);project.runs=[...newWorks,...(project.runs||[])];startProductionPlanTasks(project,newWorks);project.date='刚刚';renderProjectDetail();renderProjects();closeProjectBatchDialog();showToast(`生产计划「${name}」已新增`)};
...
function collectProjectPlanDrafts(){
  ...
  for(const [index,group] of groups.entries()){
    ...
    if(!quantity.value.trim())return invalid(quantity,'请输入计划运行次数。');if(!Number.isSafeInteger(Number(quantity.value))||Number(quantity.value)<1)return invalid(quantity,'计划运行次数须为大于0的整数。');
...
function buildInitialProjectPlans(drafts,owner,numbers){
  ...
  drafts.forEach((draft,index)=>{
    ...
    batches.push(plan);
    works.push(...createPlanWorks(plan,numbers.flow.slice(offset,offset+draft.quantity)));
    offset+=draft.quantity;
  });
  return {batches,runs:works};
}
...
function openProjectEditPage(projectId){
  ...
  const customer=businessCustomers.find(item=>item.code===(project.customerCode||order?.customerCode));
  if(customer){if(!Array.from(projectCustomer.options).some(option=>option.value===customer.code)){const option=new Option(customer.name,customer.code);projectCustomer.append(option)}projectCustomer.value=customer.code}
...
function saveProjectEdit(){
  ...
  const order=businessOrders.find(item=>item.id===project.orderId),oldCustomer=project.customerCode||order?.customerCode;
  const selectedCustomer=customerFromSelect(projectCustomer),customer=selectedCustomer.name,customerCode=selectedCustomer.code,owner=document.querySelector('#projectOwner').selectedOptions[0].textContent;
  ...
  Object.assign(project,{title:projectNameInput.value.trim(),customer,customerCode,owner,notes:sanitizeRichText(document.querySelector('#projectOrderNotes').innerHTML),references,date:new Date().toLocaleString('zh-CN',{hour12:false})});
  if(/^客户：.*负责人：/.test(project.desc))project.desc=`客户：${customer}    负责人：${owner}`;
  if(order)Object.assign(order,{name:project.title,customer,customerCode,owner,notes:project.notes,references:references.map(file=>typeof file==='string'?file:file.name)});
  if(oldCustomer&&oldCustomer!==customerCode){const previous=businessCustomers.find(item=>item.code===oldCustomer),next=selectedCustomer;if(previous){previous.projects=Math.max(0,previous.projects-1);if(order)previous.orders=Math.max(0,previous.orders-1)}next.projects+=1;if(order)next.orders+=1}
...
function openOrderDialog(source='business',trigger=businessAddButton){orderDialogSource=source;orderDialogTrigger=trigger;document.querySelector('#newOrderDescription').textContent='创建后将同步到订单列表。';newOrderForm.reset();populateCustomerSelects();orderFileList.textContent='';newOrderDialog.hidden=false;document.body.classList.add('order-dialog-open');requestAnimationFrame(()=>newOrderCustomer.focus({preventScroll:true}))}
...
function closeCustomerDialog(restoreFocus=true){clearTimeout(customerNameCheckTimer);setCustomerDemandOpen(false);newCustomerDialog.hidden=true;editingCustomerCode=null;if(newOrderDialog.hidden)document.body.classList.remove('order-dialog-open');if(restoreFocus)requestAnimationFrame(()=>customerDialogTrigger?.focus({preventScroll:true}))}
...
function openCustomerDialog(source='order',trigger=document.querySelector('#addCustomerButton'),customer=null){customerDialogSource=source;customerDialogTrigger=trigger;editingCustomerCode=source==='edit'&&customer?customer.code:null;const editing=Boolean(editingCustomerCode);const descriptions={business:'新增后将同步到客户列表。',project:'新增后将自动回填到当前项目。',order:'新增后将自动回填到当前订单。',edit:'修改客户基础信息、业务属性与推进记录。'};document.querySelector('#newCustomerTitle').textContent=editing?'编辑客户':'新增客户';document.querySelector('#newCustomerDescription').textContent=descriptions[source]||descriptions.order;saveCustomerButton.textContent=editing?'保存修改':'确认新增';newCustomerForm.reset();newCustomerNotes.innerHTML='';newCustomerDemandNote.value='';newCustomerName.removeAttribute('aria-invalid');newCustomerContact.innerHTML='<option value="">请选择对接人</option>';teamMemberRoster.filter(member=>member.enabled!==false).forEach(member=>newCustomerContact.add(new Option(member.name,member.name)));newCustomerContact.value='Huimin Zhang';customerDemandField.classList.remove('is-invalid');setCustomerDemandOpen(false);if(editing){newCustomerName.value=customer.name||'';newCustomerCompany.value=customer.company||'';newCustomerType.value=customer.type||'';newCustomerBusinessLine.value=customer.businessLine||'';customerDemandOptions.forEach(input=>input.checked=(customer.demands||[]).includes(input.value));newCustomerContact.value=customer.contact||'';newCustomerProgressStatus.value=customer.status||'';newCustomerPriority.value=customer.priority||'';newCustomerDemandNote.value=customer.demandNote||'';newCustomerNotes.innerHTML=sanitizeRichText(customer.notes||[customer.progressNotes,customer.communication].filter(Boolean).join('<br>'))}updateCustomerDemandSummary();updateCustomerNotesCount();newCustomerDialog.hidden=false;document.body.classList.add('order-dialog-open');requestAnimationFrame(()=>newCustomerName.focus({preventScroll:true}))}
...
function customerFromSelect(select){return businessCustomers.find(item=>item.code===select.value)}
...
newCustomerForm.onsubmit=event=>{
  ...
  if(editingCustomer){
    ...
    businessOrders.forEach(order=>{if(order.customerCode===editingCustomer.code)order.customer=name});
    projectData.forEach(project=>{if(project.customerCode===editingCustomer.code)project.customer=name});
    populateCustomerSelects();
    renderBusiness();
...
projectCreateForm.onsubmit=event=>{
  ...
  const selectedCustomer=customerFromSelect(projectCustomer),existingProject=projectData.find(item=>item.id===editingProjectId);
  if(!selectedCustomer||(selectedCustomer.enabled===false&&existingProject?.customerCode!==selectedCustomer.code))return invalidateProjectField(projectCustomer,'请选择启用的客户');
  ...
  const numbers=reserveDailyProductionNumbers({project:1,plan:drafts.length,flow:totalQuantity});
  if(!numbers)return;
  ...
  businessOrders[0].customerCode=selectedCustomer.code;
  ...
  projectData[0].customerCode=selectedCustomer.code;
  projectData[0].owner=ownerName;
  startProductionPlanTasks(projectData[0],projectData[0].runs);
  renderBusiness();
