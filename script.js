...
try{const names=JSON.parse(localStorage.getItem(projectWorkNamesStorageKey)||'{}');projectData.forEach(project=>(project.works||[]).forEach(work=>{const name=names[JSON.stringify([project.id,String(work.id)])];if(typeof name==='string'&&name.trim())work.name=name}))}catch{}
function saveProjectWorkName(project,work,value){
  ...
  try{const names=JSON.parse(localStorage.getItem(projectWorkNamesStorageKey)||'{}');names[JSON.stringify([project.id,String(work.id)])]=name;localStorage.setItem(projectWorkNamesStorageKey,JSON.stringify(names))}catch{showToast('作品名称保存失败，请稍后重试','error');return false}
  work.name=name;work.updated='刚刚';
...
function projectWorkStatusClass(status){return status==='已完成'?'completed':status==='待审核'?'review':'making'}
...
function loadProductionTasks(){
  ...
  if(featuredWork){
    ...
    if(context&&firstNode){
      ...
      context.work.status='制作中';context.work.stage=firstNode.title;context.work.updated='刚刚';
    }
  }
  normalizeProductionTaskConcurrency();
  projectData.forEach(project=>ensureAssignedProductionTasks(project,(project.works||[]).filter(work=>Object.keys(work.nodeAssignments||{}).length)));
  persistProductionTasks();
}
function ensureAssignedProductionTasks(project,works){
  ...
  works.forEach(work=>{
    ...
    work.status='制作中';work.stage=active.nodeTitle;work.updated='刚刚';
  });
  return created;
}
...
function productionTaskRow(task){
  ...
  return `<div class="task-item-wrap${task.status==='in_progress'?' is-editable':''}"><button class="task-item${active?' active':''}${task.featured?' featured':''}" type="button" data-task-id="${escapeHtml(task.id)}" aria-current="${active?'true':'false'}"><span class="task-item-head"><i class="task-node-icon ${escapeHtml(task.nodeCategory)}" aria-hidden="true">${category.symbol}</i><span class="task-item-title"><b>${escapeHtml(task.workName)}${task.featured?'<em>完整流程</em>':''}</b><small>${escapeHtml(task.nodeTitle)} · 第 ${task.nodeIndex+1}/${task.nodeCount} 节点</small></span><span class="task-status ${status.className}">${status.label}</span></span><span class="task-item-body"><span><b>${escapeHtml(task.projectName)}</b><small>${escapeHtml(task.planName)} · ${escapeHtml(task.workflowName)} ${escapeHtml(task.workflowVersion)}</small></span></span></button>${task.status==='in_progress'?`<button class="task-rename-button" type="button" data-edit-task-name="${escapeHtml(task.id)}" title="编辑任务名称" aria-label="编辑任务名称：${escapeHtml(task.workName)}"><svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.6" aria-hidden="true"><path d="m15 5 4 4M4 20l4-1L20 7a2.8 2.8 0 0 0-4-4L4 15z"/></svg></button>`:''}</div>`;
}
  ...
  if(outcome==='complete'){
    if(['lyricsReview','songReview'].includes(task.nodeType)){
      ...
      if(reviewResult==='rejected'){
        ...
        context.work.status='制作中';context.work.stage=sourceTask.nodeTitle;context.work.updated='刚刚';selectedTaskId=sourceTask.id;
        normalizeProductionTaskConcurrency();
    ...
    if(nextNode){
      ...
      context.work.status='制作中';context.work.stage=nextNode.title;context.work.updated='刚刚';selectedTaskId=nextTask.id;
    }else{context.work.status='已完成';context.work.stage='已完成';context.work.updated='刚刚';selectedTaskId=task.id}
  }else{context.work.status='已终止';context.work.stage='已终止';context.work.updated='刚刚';selectedTaskId=task.id}
  normalizeProductionTaskConcurrency();
...
function cancelTaskNameEdit(form){const wrap=form.closest('.task-item-wrap');form.remove();const button=wrap.querySelector('[data-edit-task-name]');button.hidden=false;button.focus({preventScroll:true})}
taskList.addEventListener('click',event=>{
  const edit=event.target.closest('[data-edit-task-name]');
  if(edit){
    const task=productionTasks.find(item=>item.id===edit.dataset.editTaskName);if(!task||task.status!=='in_progress')return;
    const form=document.createElement('form');form.className='task-name-editor';form.dataset.taskNameForm=task.id;form.noValidate=true;
    form.innerHTML=`<label>任务名称<input data-task-name-input aria-label="任务名称" value="${escapeHtml(task.workName)}" placeholder="请输入任务名称" /></label><div><button type="submit">保存</button><button type="button" data-cancel-task-name>取消</button></div>`;
    edit.hidden=true;edit.closest('.task-item-wrap').append(form);form.querySelector('input').focus();form.querySelector('input').select();return;
  }
  const cancel=event.target.closest('[data-cancel-task-name]');if(cancel){cancelTaskNameEdit(cancel.closest('form'));return}
  if(event.target.closest('[data-task-name-form]'))return;
...
taskList.addEventListener('submit',event=>{
  const form=event.target.closest('[data-task-name-form]');if(!form)return;event.preventDefault();const input=form.querySelector('[data-task-name-input]');
  if(!renameProductionTask(form.dataset.taskNameForm,input.value)){input.setAttribute('aria-invalid','true');input.focus();return}
  saveTaskPageValues(productionTasks.find(item=>item.id===selectedTaskId));persistProductionTasks();
  const taskId=form.dataset.taskNameForm;renderTaskCenter();
  const project=projectData.find(item=>item.id===selectedProjectId);if(project)renderProjectWorks(project);
  const button=Array.from(taskList.querySelectorAll('[data-edit-task-name]')).find(item=>item.dataset.editTaskName===taskId);(button||taskSearch).focus({preventScroll:true});showToast('任务名称已保存');
});
taskList.addEventListener('keydown',event=>{if(event.target.matches('[data-task-name-input]')&&!event.isComposing){if(event.key==='Escape'){event.preventDefault();event.stopPropagation();cancelTaskNameEdit(event.target.closest('form'))}}});
...
function reserveDailyProductionNumbers(counts,now=new Date()){
  ...
  const prefixes={project:`PROJECT-${day}-`,plan:`BATCH-${day}-`,flow:`FLOW${day}`};
  const existing={project:projectData.map(project=>project.id),plan:projectData.flatMap(project=>(project.batches||[]).map(plan=>plan.id)),flow:projectData.flatMap(project=>(project.works||[]).map(work=>work.serial))};
  try{
    ...
    Object.entries(counts).forEach(([kind,count])=>{
      ...
      if(!Number.isSafeInteger(start+count))throw Error('编号已超出范围');
      result[kind]=Array.from({length:count},(_,index)=>prefix+String(start+index+1).padStart(5,'0'));
...
function createPlanWorks(plan,reservedSerials){
  ...
  const now=new Date(),serials=reservedSerials||reserveDailyProductionNumbers({flow:quantity},now)?.flow;
  if(!serials||serials.length!==quantity)throw Error('流水编号生成失败');
  return serials.map(serial=>({id:serial,serial,name:`未命名-${serial.slice(-4)}`,planId:plan.id,planName:plan.name,status:'制作中',stage:'基于参考生成歌词',lyrics:'暂无',audio:'暂无',cover:'暂无',owner:'未分配',createdAt:now.toISOString(),updated:'刚刚'}));
}
...
projectCreateForm.onsubmit=event=>{
  ...
  if(projectData.some(item=>item.title===projectName))return invalidateProjectField(projectNameInput,'该项目名称已存在');
  ...
  const numbers=reserveDailyProductionNumbers({project:1,plan:1,flow:Number(quantity.value)});
  if(!numbers)return;
  ...
  projectData.unshift({id:numbers.project[0],title:projectName,desc:description,type:'生产项目',progress:0,date:'刚刚',color:'linear-gradient(135deg,#5c3bd1,#b64f88)',members:['H'],scope:'owned',risk:false,batches:[initialBatch],works:createPlanWorks(initialBatch,numbers.flow),orderId,quantity:Number(quantity.value),workflowId:workflow.id,deliveryDate,notes:orderNotes,productionPlanNotes});
  renderBusiness();
